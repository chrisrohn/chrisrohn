...
import { sourceLabel } from "./feed.js";
...
/** @param {Record<string, {n: number, k: number, rate: number}>} table @param {number} [top] @param {(name: string) => string} [label] */
function rates(table, top = 12, label = name => name.includes(":") ? name.split(":").slice(1).join(":") : name) {
  ...
  return rows.map(([name, r]) => bar(label(name), r.rate, 1, `${Math.round(r.rate * 100)}% · ${r.k}/${r.n}`)).join("");
}
