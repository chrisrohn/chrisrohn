...
export function noticeDupes() {
  ...
  setTimeout(() => toast(`⚠ ${dupCount()} duplicated songs in the year playlists`, true, { label: "Review", fn: openCleanup }), 2500);
}
/** Switch to the Cleanup tab (from the toast, from Settings). */
export function openCleanup() { const tab = $(".tab[data-view=cleanup]"); if (tab) tab.click(); }
...
export function dupesSummary() {
  ...
  if (!dupCount()) return `Duplicate check: none found by the last build (${when})`;
  const done = dupesLoaded() ? dupCount() - openDupes() : 0;
  return `${dupCount()} duplicated songs in the year playlists (checked ${when})${done ? ` · ${done} cleaned since` : ""}`;
}
export async function renderDupes(reset = true) {
  ...
  if (!dupCount()) { box.innerHTML = ""; $("#cl-summary").textContent = dupesSummary(); return; }
  ...
  const pill = $("#count-cleanup"); if (pill) pill.textContent = String(openDupes());
  ...
  $("#cl-dupe-kinds").textContent = Object.entries(kinds).map(([k, n]) => `${n} ${KIND[k] || k}`).join(" · ");
