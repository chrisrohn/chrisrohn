...
/** Accents folded, & as "and", punctuation to spaces: the same normalisation the build uses (discovery/util.norm). @param {string | null | undefined} s */
export const norm = s => String(s || "").toLowerCase().normalize("NFKD").replace(/[̀-ͯ]/g, "").replace(/&/g, " and ").replace(/[^\p{L}\p{N}]+/gu, " ").trim();
/** @param {Date} d */
...
/** A quiet tap on the phone's motor for a decision, where the browser allows it. @param {number} [ms] */
export function buzz(ms = 12) { try { if (navigator.vibrate) navigator.vibrate(ms); } catch { /* ignore */ } }

/** The card's own address on this site (?t=<id>). @param {string} id */
 ...
 * What travels is the card's own page here (it opens on the card, and plays), with the YouTube link in the text. */
