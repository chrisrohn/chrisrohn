...
| **Verify** one video | `GET playlistItems` (`playlistId` + `videoId`) | 1 unit | Right before an add, when the reading above is stale or the target is another year's playlist; before every Cleanup removal to find the exact items; and before a Video tab approval, unless the music-video playlist was read in the last half hour | `playlistItemsFor` in `site/src/youtube.js`, used by `site/src/rating.js`, `site/src/dupes.js` and `site/src/videos.js` |
...
| **Add** an approved video | `POST playlistItems` | 50 units | Only when the curator presses ▲︎ add on the Video tab: the official video of a song the library holds is added to the one music-video playlist, never to a year playlist | `addToPlaylist` ← `approve()` in `site/src/videos.js` |
| **Remove** a track | `DELETE playlistItems` | 50 units | Undo within seconds of a Keep; *restore* on the Skipped tab; Cleanup: the extra copy of a song added twice, a copy the curator chose not to keep, a copy moved to another year, or a copy that will not play here (swapped for the upload that does); Undo of a Video tab approval | `removePlaylistItem` ← `undo()`, `restoreAll()` in `rating.js`; `removeRow`, `moveRow`, `swapTrack` in `dupes.js`; `undoVideo()` in `videos.js` |
...
| **Read** a playlist | `GET playlistItems` (`playlistId`, paged) | 1 unit / 50 | Cleanup, on a tap: the live scan of one year (duplicates as of now) and the playability audit of one year; the Video tab, when it opens and at most every 30 min: the music-video playlist, so videos it already holds are hidden and an approval never adds a second copy | `scanYear` in `dupes.js`, `auditYear` in `audit.js`, `refreshHeld` in `videos.js` |
