"""python -m discovery <command>
  ...
  videos            the video side of every song the year playlists hold, for the Video tab → site/data/videos.json
...
def _parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("videos", help="build the Video tab's list (site/data/videos.json): the official videos of the songs the year playlists hold")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if cmd == "concerts":
        ...
        return 0
    if cmd == "videos":
        from .videos import build_videos
    ...
    if cmd == "daily":
        ...
        if (cfg.get("videos") or {}).get("in_daily", False):
            ...
            build_videos(cfg, deadline_minutes=max(0.0, left))
        return 0
