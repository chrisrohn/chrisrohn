...
- **⚙ → API activity**: every YouTube Data API request the browser makes, as it happens, with what it was for (verify a playlist's contents, add the approved track, remove one taken back), its result and quota cost, exportable as text or JSON; [docs/youtube-api-compliance.md](docs/youtube-api-compliance.md) turns it into the screencast Google's quota reviewers ask for, and [docs/youtube-tos-reply.md](docs/youtube-tos-reply.md) is the reply to their ToS report (branding, project numbers, how long API data is kept)
...
- A **Concerts** tab, the Songkick page rebuilt from open sources: every upcoming show within 80 miles of Detroit
  ...
  Edmtrain (each with a free key) and JamBase (its Developer tier: six months scanned in date bands, the near future often
  and the far end rarely, a ledger in the state holding it under its 1,000 calls a month, and its credit on the tab) — so the clubs that sell through DICE, Etix or their own box office
...
- A **Video** tab, the one place videos live: the year playlists hold audio tracks, and the official video YouTube Music pairs with each of them is reviewed here — every song in every year playlist (the **Videos** workflow asks YouTube Music about the library a batch a day, free, until every row has been asked) and every song kept from the feed whose card knows its video (the resolver asks as it resolves, so a Keep brings the video the same day). Each video is a card like the feed's — it plays in the site's player, the list is the queue, and the keys, the swipe and the player's own thumbs work as on the Feed; on a phone the tab is the Feed's deck (one video at a time, the player as the card's picture, ▼︎ pass / ▶ / ▲︎ add pinned at the foot); **▲︎** puts the video into the single music-video playlist ([Indie Discotheque · videos](https://music.youtube.com/playlist?list=PLTW5JZnPjE_r0Y2WwYFLFAbEVZTlt6xuX)), the only thing the tab ever writes, and **▼︎** passes for free; decisions travel with the ratings to every device and to the build, and what the playlist already holds is hidden
...
- Songs only: nothing under 1:55 (interludes, skits, intros) or over 9:31 (DJ mixes, full sides) becomes a card, in the feed or the catalog; between several uploads of a song the resolver takes the one of song length (the radio edit over the extended mix)
- A year for everything it can: each track is identified as a recording and dated by MusicBrainz, Discogs, Deezer and Apple Music, the earliest original release winning; when none of them knows, the release date YouTube Music states for the upload itself stands in as a weak hint rather than leaving the card at *year?*
...
- A day is 500 songs: the build ranks up to `ranking.max_items` (500) cards a day, the Feed opens on a shortlist of them and *show all* lifts it; the candidates are picked with the resolver's cache in hand, so a slot never goes to a song the cache already knows is not a card today (a video with no audio side, a miss not yet due, a song already filed) and the YouTube lookup budget is spent on sightings that can join the day
